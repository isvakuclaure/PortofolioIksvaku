...
export default function Aficiones() {
  ...
  // Scroll-spy: on each scroll tick find which section top is at/above viewport mid-point
  useEffect(() => {
    const OFFSET = 56 + 48; // navbar height + tab bar height

    const handleScroll = () => {
      const threshold = window.scrollY + OFFSET + 40;
      let current = 0;
    ...
    handleScroll(); // set correct tab on mount
  ...
  const handleTabClick = (i: number) => {
    ...
    if (!el) return;
    // Offset for sticky navbar (56px) + sticky tab bar (~48px)
    const offset = 56 + 48;
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          {tabs.map((tab, i) => (
            <section
              ...
              className="bg-white rounded-xl border border-gray-200 overflow-hidden border-l-[3px] border-l-blue-500"
            >
              <div className="p-6 grid md:grid-cols-2 gap-8 items-start">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed">{tab.text}</p>
                </div>
              </div>
            </section>
          ))}
